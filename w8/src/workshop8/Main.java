...
public class Main {
	...
	public static void main(String args[]) {
		...
		Student[] s = { new Student("Yuki", "Waka", 100.0, "IT"), new Student("Gihoon", "Kim", 76.0, "IT"),
				new Student("Jin", "Kim", 35.8, "Business"), new Student("Eri", "Hashimoto", 47.0, "Media"),
				new Student("Eric", "Hashimoto", 43.0, "IT"), new Student("Ian", "Kwon", 87.0, "Accounting"), 
		...
		list.sort(StudentProcess.NameComparator);
		StudentProcess.fullName(list).forEach(lName -> System.out.println(lName));
		
		System.out.println("\nTask 7: \n\nStudents by department:");
		list.sort((StudentProcess.NameComparator).reversed());
		StudentProcess.department(list).forEach( (dep, lists) -> {
            System.out.println(dep);
            lists.forEach(e -> System.out.println("     " + e));
        });
		System.out.println("\nTask 8: \n\nCount of Students by department:");
		list.sort((StudentProcess.NameComparator).reversed());
		StudentProcess.count(list).forEach( (dep, lists) -> {
         
			System.out.println(dep+" has " +lists+" Student(s)");
		});
		list.sort(StudentProcess.NameComparator);
		System.out.printf("\nTask 9: \n\nSum of Students' grades: %.2f ", StudentProcess.grade(list).sum());
		
		list.sort(StudentProcess.NameComparator);
		System.out.printf("\n\nTask 10: \n\nAverage of Students' grades:  %.2f", StudentProcess.grade(list).average().getAsDouble());
